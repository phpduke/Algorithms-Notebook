...
// reflection of point p on line ab
PT PointReflectByLine(PT a,PT b,PT p) {
    PT ret=ProjectPointLine(a,b,p);
    return (ret+ret-p);
}
...
// is point a on segment bc
bool PointOnSegment(PT a,PT b,PT c) {
    if (!LinesParallel(a,b,a,c)) return false;
    if (b.x<=a.x && a.x<=c.x) return true;
    if (c.x<=a.x && a.x<=b.x) return true;
    return false;
}
